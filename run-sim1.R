...
for(i in 1:NITER){
  ...
  for(run in c(1, 2)){
    ...
    ys = xbeta + rnorm(length(xbeta), 0, 30)
...
write.table(crres, "crres_2000x2.csv", sep="\t", quote=F, col.names=T, row.names=F)
write.table(summaries, "studysummaries_2000x2.csv", sep="\t", quote=F, col.names=T, row.names=F)
